...
function generateOptionsHtml(settings: UserSettingsInput, manifest: ManifestInput): string {
  ...
  for (const [key, config] of Object.entries(settings)) {
    ...
    if (type === 'checkbox' || type === 'radio') {
      ...
      html += '            \n';
      html += '            \n';
      html += '            <div id="panel-min-' + key + '" class="time24h-panel"' +
        ' style="display: none; position: absolute; top: 32px; left: 50px;' +
        ' background: #1a1a1a; border: 1px solid #00ff00; grid-template-columns: repeat(10, 1fr); gap: 4px; padding: 6px; z-index: 100; overflow-y: auto; border-radius: 4px;">\n';
